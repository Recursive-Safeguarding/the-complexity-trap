...
from sweagent.agent.history_processors import DefaultHistoryProcessor, HistoryProcessor, SummarizeEveryNTurns
...
from sweagent.types import AgentInfo, AgentRunResult, InstanceStats, StepOutput, Trajectory, TrajectoryStep
...
class TemplateConfig(BaseModel):
    ...
    summary_system_template: str = """You are maintaining a context-aware state summary for an interactive agent working on software engineering tasks (specifically, bug fixes within the SWE-bench framework). You will be given a list of turns corresponding to actions taken by the agent and their resulting observations, and the most recent previous summary if one exists. Track:
...
class DefaultAgentConfig(BaseModel):
    ...
    summary_model: ModelConfig = Field(
        # This default and its exact name are used to handle the case of reusing the agent's main model for summarization.
        default_factory=lambda: GenericAPIModelConfig(name='reuse-agent-model'),
        description=(
            "Configuration for the model used for summarization in history processors "
            "like SummarizeEveryNTurns. If None, defaults to the agent's main model."
        ),
    )
...
class RetryAgent(AbstractAgent):
    ...
    def _finalize_agent_run(self) -> None:
        ...
        self._total_instance_attempt_stats += self._agent.stats
...
class DefaultAgent(AbstractAgent):
    ...
    def from_config(cls, config: DefaultAgentConfig) -> Self:
        ...
        if config.summary_model.name == "reuse-agent-model":
            summary_model = model  # Use the main agent model if no specific summary model is configured
            get_logger("swea-agent", emoji="🤠").info(
                f"Using main agent model for summarization: {config.model.name}"
            )
        else:
        ...
        # Share instance stats between main and summary models for consistent accounting.
        try:
    ...
    def setup(
        ...
        self._uses_llm_summaries = any(isinstance(hp, SummarizeEveryNTurns) for hp in self.history_processors)
        self._llm_summary_processor = [hp for hp in self.history_processors if isinstance(hp, SummarizeEveryNTurns)][0] if self._uses_llm_summaries else None
        ...
        attempt_data = copy.deepcopy(
            {
                ...
                "summaries": [summary.model_dump() for summary in self._llm_summary_processor.summaries] if self._uses_llm_summaries else None,
                "info": self.info
            }
        )
        ...
        trajectory_step = TrajectoryStep(
            {
                ...
                "state": step.state,
                "turn_statistics": step.turn_statistics.model_dump() if step.turn_statistics is not None else None,
